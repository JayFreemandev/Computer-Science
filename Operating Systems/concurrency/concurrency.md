...
교착 상태의 발생 원인과 대응방법에대해 살펴보았다. 이 문제는 병행성 문제 자체만큼이나 오래된 것이라 
관련 주제에 대해수백 편의 논문들이 소개되었다. 가장 좋은 해법은 조심하는 것.  
그리고 락 획득 순서를 정해서애초에 교착 상태가 발생하지 않도록 예방하는 것이다.    

Linux를 포함해서 보편적으로사용되는 라이브러리나 중요한 시스템에서 대기없는 자료 구조가 사용되는 것을 보아   
노대기 기법들도 유망해 보인다. 하지만, 보편성이 부족하고 이 방법은 자료구조를 만드는 것이 복잡하기 때문에 제한적으로 사용될 것으로 보인다.   
